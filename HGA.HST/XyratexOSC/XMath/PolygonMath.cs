...
{
    /// <summary>
    /// Performs the mathematical calculations on <see cref="Polygon"/> objects.
    /// </summary>
    public class PolygonMath : PolygonMathBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PolygonMath"/> class.
        /// </summary>
        public PolygonMath()
        {
            m_Scanbeam = null;
        ...
        {
            if (m_edges.Count == 0) return; //avoids problems with PolygonMathBase destructor
            DisposeAllPolyPts();
            base.Clear();
        }
        //------------------------------------------------------------------------------
        ...
        private void InsertScanbeam(Int64 Y)
        {
            ...
            else
            {
                ...
                if (Y == sb2.Y) return; //ignores duplicates
                Scanbeam newSb = new Scanbeam();
                newSb.Y = Y;
                newSb.next = sb2.next;
                sb2.next = newSb;
        ...
        internal void FixHoleLinkage(OutRec outRec)
        {
            ...
            if (outRec == tmp) throw new Exception("HoleLinkage error");
        ...
        private void AddJoin(TEdge e1, TEdge e2, int e1OutIdx, int e2OutIdx)
        {
            ...
            jr.pt1a = new IntPoint2D(e1.xcurr, e1.ycurr);
            jr.pt1b = new IntPoint2D(e1.xtop, e1.ytop);
            if (e2OutIdx >= 0)
                jr.poly2Idx = e2OutIdx;
            else
                jr.poly2Idx = e2.outIdx;
            jr.pt2a = new IntPoint2D(e2.xcurr, e2.ycurr);
            jr.pt2b = new IntPoint2D(e2.xtop, e2.ytop);
            m_Joins.Add(jr);
        }
        //------------------------------------------------------------------------------
        ...
        private void InsertLocalMinimaIntoAEL(Int64 botY)
        {
            while (m_CurrentLM != null && (m_CurrentLM.Y == botY))
            {
                ...
                if (IsContributing(lb))
                    AddLocalMinPoly(lb, rb, new IntPoint2D(lb.xcurr, m_CurrentLM.Y));
                ...
                if (rb.outIdx >= 0)
                {
                    if (rb.dx == horizontal)
                    {
                        ...
                        {
                            IntPoint2D pt = new IntPoint2D(), pt2 = new IntPoint2D(); //used as dummy params.
                            HorzJoinRec hj = m_HorizJoins[i];
                            //if horizontals rb and hj.edge overlap, flag for joining later ...
                            if (GetOverlapSegment(new IntPoint2D(hj.edge.xbot, hj.edge.ybot),
                                new IntPoint2D(hj.edge.xtop, hj.edge.ytop),
                                new IntPoint2D(rb.xbot, rb.ybot),
                                new IntPoint2D(rb.xtop, rb.ytop),
                                ref pt, ref pt2))
                ...
                if (lb.nextInAEL != rb)
                {
                    ...
                    IntPoint2D pt = new IntPoint2D(lb.xcurr, lb.ycurr);
                    while (e != rb)
                    {
                        if (e == null)
                            throw new Exception("InsertLocalMinimaIntoAEL: missing rightbound!");
        ...
        private void AddLocalMaxPoly(TEdge e1, TEdge e2, IntPoint2D pt)
        {
            AddOutPt(e1, pt);
        ...
        private void AddLocalMinPoly(TEdge e1, TEdge e2, IntPoint2D pt)
        {
            TEdge e, prevE;
        ...
        private void AddOutPt(TEdge e, IntPoint2D pt)
        {
        ...
        internal void SwapPoints(ref IntPoint2D pt1, ref IntPoint2D pt2)
        {
            IntPoint2D tmp = pt1;
            pt1 = pt2;
            pt2 = tmp;
        ...
        private bool GetOverlapSegment(IntPoint2D pt1a, IntPoint2D pt1b, IntPoint2D pt2a,
            IntPoint2D pt2b, ref IntPoint2D pt1, ref IntPoint2D pt2)
        {
        ...
        private bool FindSegment(ref OutPt pp, ref IntPoint2D pt1, ref IntPoint2D pt2)
        {
            if (pp == null) return false;
            OutPt pp2 = pp;
            IntPoint2D pt1a = new IntPoint2D(pt1);
            IntPoint2D pt2a = new IntPoint2D(pt2);
            do
            {
        ...
        internal bool Pt3IsBetweenPt1AndPt2(IntPoint2D pt1, IntPoint2D pt2, IntPoint2D pt3)
        {
        ...
        private OutPt InsertPolyPtBetween(OutPt p1, OutPt p2, IntPoint2D pt)
        {
        ...
        private double GetDx(IntPoint2D pt1, IntPoint2D pt2)
        {
        ...
        private void DoEdge1(TEdge edge1, TEdge edge2, IntPoint2D pt)
        {
            AddOutPt(edge1, pt);
        ...
        private void DoEdge2(TEdge edge1, TEdge edge2, IntPoint2D pt)
        {
            AddOutPt(edge2, pt);
            SwapSides(edge1, edge2);
            SwapPolyIndexes(edge1, edge2);
        ...
        private void DoBothEdges(TEdge edge1, TEdge edge2, IntPoint2D pt)
        {
            AddOutPt(edge1, pt);
            AddOutPt(edge2, pt);
            SwapSides(edge1, edge2);
        ...
        private void IntersectEdges(TEdge e1, TEdge e2, IntPoint2D pt, Protects protects)
        {
        ...
        private void ProcessHorizontal(TEdge horzEdge)
        {
                  ...
                  ((Direction == Direction.dRightToLeft) && (e.xcurr >= horzLeft)))
                {
                    ...
                    if (e == eMaxPair)
                    {
                        ...
                        if (Direction == Direction.dLeftToRight)
                            IntersectEdges(horzEdge, e, new IntPoint2D(e.xcurr, horzEdge.ycurr), 0);
                        else
                            IntersectEdges(e, horzEdge, new IntPoint2D(e.xcurr, horzEdge.ycurr), 0);
                        if (eMaxPair.outIdx >= 0) throw new Exception("ProcessHorizontal error");
                        return;
                    }
                    else if (e.dx == horizontal && !IsMinima(e) && !(e.xcurr > e.xtop))
                    {
                        if (Direction == Direction.dLeftToRight)
                            IntersectEdges(horzEdge, e, new IntPoint2D(e.xcurr, horzEdge.ycurr),
                              (IsTopHorz(horzEdge, e.xcurr)) ? Protects.ipLeft : Protects.ipBoth);
                        else
                            IntersectEdges(e, horzEdge, new IntPoint2D(e.xcurr, horzEdge.ycurr),
            ...
            if (horzEdge.nextInLML != null)
            {
                if (horzEdge.outIdx >= 0)
                    AddOutPt(horzEdge, new IntPoint2D(horzEdge.xtop, horzEdge.ytop));
            ...
            else
            {
                if (horzEdge.outIdx >= 0)
                    IntersectEdges(horzEdge, eMaxPair,
                        new IntPoint2D(horzEdge.xtop, horzEdge.ycurr), Protects.ipBoth);
        ...
        private Int64 TopX(IntPoint2D pt1, IntPoint2D pt2, Int64 currentY)
        {
        ...
        private void AddIntersectNode(TEdge e1, TEdge e2, IntPoint2D pt)
        {
        ...
        private bool IntersectPoint(TEdge edge1, TEdge edge2, ref IntPoint2D ip)
        {
            double b1, b2;
        ...
        private void ProcessEdgesAtTopOfScanbeam(Int64 topY)
        {
            ...
            while (e != null)
            {
                ...
                else
                {
                    ...
                    if (IsIntermediate(e, topY) && e.nextInLML.dx == horizontal)
                    {
                        if (e.outIdx >= 0)
                        {
                            AddOutPt(e, new IntPoint2D(e.xtop, e.ytop));

                            for (int i = 0; i < m_HorizJoins.Count; ++i)
                            {
                                IntPoint2D pt = new IntPoint2D(), pt2 = new IntPoint2D();
                                HorzJoinRec hj = m_HorizJoins[i];
                                if (GetOverlapSegment(new IntPoint2D(hj.edge.xbot, hj.edge.ybot),
                                    new IntPoint2D(hj.edge.xtop, hj.edge.ytop),
                                    new IntPoint2D(e.nextInLML.xbot, e.nextInLML.ybot),
                                    new IntPoint2D(e.nextInLML.xtop, e.nextInLML.ytop), ref pt, ref pt2))
            ...
            while (e != null)
            {
                ...
                {
                    if (e.outIdx >= 0) AddOutPt(e, new IntPoint2D(e.xtop, e.ytop));
                    UpdateEdgeIntoAEL(ref e);

                    //if output polygons share an edge, they'll need joining later ...
                    if (e.outIdx >= 0 && e.prevInAEL != null && e.prevInAEL.outIdx >= 0 &&
                      e.prevInAEL.xcurr == e.xbot && e.prevInAEL.ycurr == e.ybot &&
                      SlopesEqual(new IntPoint2D(e.xbot, e.ybot), new IntPoint2D(e.xtop, e.ytop),
                        new IntPoint2D(e.xbot, e.ybot),
                        new IntPoint2D(e.prevInAEL.xtop, e.prevInAEL.ytop), m_UseFullRange))
                    {
                        AddOutPt(e.prevInAEL, new IntPoint2D(e.xbot, e.ybot));
                      ...
                      SlopesEqual(new IntPoint2D(e.xbot, e.ybot), new IntPoint2D(e.xtop, e.ytop),
                        new IntPoint2D(e.xbot, e.ybot),
                        new IntPoint2D(e.nextInAEL.xtop, e.nextInAEL.ytop), m_UseFullRange))
                    {
                        AddOutPt(e.nextInAEL, new IntPoint2D(e.xbot, e.ybot));
        ...
        private void DoMaxima(TEdge e, Int64 topY)
        {
            ...
            {
                if (eNext == null) throw new Exception("DoMaxima error");
                IntersectEdges(e, eNext, new IntPoint2D(X, topY), Protects.ipBoth);
            ...
            {
                IntersectEdges(e, eMaxPair, new IntPoint2D(X, topY), Protects.ipNone);
            }
        ...
        /// <returns></returns>
        /// <exception cref="System.Exception">Coordinate exceeds range bounds.</exception>
        public static bool Orientation(Polygon poly)
        {
            ...
            IntPoint2D vec1 = new IntPoint2D(poly[j].X - poly[jminus].X, poly[j].Y - poly[jminus].Y);
            IntPoint2D vec2 = new IntPoint2D(poly[jplus].X - poly[j].X, poly[jplus].Y - poly[j].Y);
                ...
                Math.Abs(vec2.X) > loRange || Math.Abs(vec2.Y) > loRange)
            {
                if (Math.Abs(vec1.X) > hiRange || Math.Abs(vec1.Y) > hiRange ||
                    ...
                    throw new Exception("Coordinate exceeds range bounds.");
        ...
        private bool Orientation(OutRec outRec, bool UseFull64BitRange)
        {
            ...
            IntPoint2D vec1 = new IntPoint2D(op.pt.X - opPrev.pt.X, op.pt.Y - opPrev.pt.Y);
            IntPoint2D vec2 = new IntPoint2D(opNext.pt.X - op.pt.X, opNext.pt.Y - op.pt.Y);
        ...
        private void JoinCommonEdges(bool fixHoleLinkages)
        {
            for (int i = 0; i < m_Joins.Count; i++)
            {
                ...
                IntPoint2D pt1 = new IntPoint2D(j.pt2a);
                IntPoint2D pt2 = new IntPoint2D(j.pt2b);
                IntPoint2D pt3 = new IntPoint2D(j.pt1a);
                IntPoint2D pt4 = new IntPoint2D(j.pt1b);
        ...
        internal static Polygon BuildArc(IntPoint2D pt, double a1, double a2, double r)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                result.Add(new IntPoint2D(pt.X + Round(Math.Cos(a) * r), pt.Y + Round(Math.Sin(a) * r)));
                a += da;
            }
            return result;
        }
        ...
        internal static DoublePoint GetUnitNormal(IntPoint2D pt1, IntPoint2D pt2)
        {
        ...
        private class PolyOffsetBuilder
        {
            ...
            public PolyOffsetBuilder(Polygons pts, Polygons solution, double delta, JoinType jointype, double MiterLimit = 2)
            {
                ...
                PolygonMath pm = new PolygonMath();
                pm.AddPolygons(solution, PolyType.ptSubject);
                if (delta > 0)
                {
                    pm.Execute(ClipType.ctUnion, solution, PolyFillType.pftPositive, PolyFillType.pftPositive);
                }
                else
                {
                    IntRegion2D r = pm.GetBounds();
                    Polygon outer = new Polygon(4);

                    outer.Add(new IntPoint2D(r.Left - 10, r.Bottom + 10));
                    outer.Add(new IntPoint2D(r.Right + 10, r.Bottom + 10));
                    outer.Add(new IntPoint2D(r.Right + 10, r.Top - 10));
                    outer.Add(new IntPoint2D(r.Left - 10, r.Top - 10));

                    pm.AddPolygon(outer, PolyType.ptSubject);
                    pm.Execute(ClipType.ctUnion, solution, PolyFillType.pftNegative, PolyFillType.pftNegative);
                    if (solution.Count > 0)
                    {
                        solution.RemoveAt(0);
            ...
            internal void AddPoint(IntPoint2D pt)
            {
            ...
            {
                IntPoint2D pt1 = new IntPoint2D((Int64)Round(pts[m_i][m_j].X + normals[m_k].X * delta),
                    (Int64)Round(pts[m_i][m_j].Y + normals[m_k].Y * delta));
                IntPoint2D pt2 = new IntPoint2D((Int64)Round(pts[m_i][m_j].X + normals[m_j].X * delta),
                ...
                if ((normals[m_k].X * normals[m_j].Y - normals[m_j].X * normals[m_k].Y) * delta >= 0)
                {
                    ...
                    pt1 = new IntPoint2D((Int64)(pt1.X - normals[m_k].Y * dx),
                        (Int64)(pt1.Y + normals[m_k].X * dx));
                    AddPoint(pt1);
                    pt2 = new IntPoint2D((Int64)(pt2.X + normals[m_j].Y * dx),
            ...
            internal void DoMiter()
            {
                ...
                {
                    double q = delta / m_R;
                    AddPoint(new IntPoint2D((Int64)Round(pts[m_i][m_j].X +
            ...
            {
                IntPoint2D pt1 = new IntPoint2D(Round(pts[m_i][m_j].X + normals[m_k].X * delta),
                    Round(pts[m_i][m_j].Y + normals[m_k].Y * delta));
                IntPoint2D pt2 = new IntPoint2D(Round(pts[m_i][m_j].X + normals[m_j].X * delta),
              ...
              PolyFillType fillType = PolyFillType.pftEvenOdd)
        {
            ...
            PolygonMath pm = new PolygonMath();
            pm.AddPolygon(poly, PolyType.ptSubject);
            pm.Execute(ClipType.ctUnion, result, fillType, fillType);
            return result;
        }
        //------------------------------------------------------------------------------
            ...
            PolyFillType fillType = PolyFillType.pftEvenOdd)
        {
            Polygons result = new Polygons();
            PolygonMath pm = new PolygonMath();
            pm.AddPolygons(polys, PolyType.ptSubject);
            pm.Execute(ClipType.ctUnion, result, fillType, fillType);
            return result;
        }
        //------------------------------------------------------------------------------
    ...
    public class PolygonMathBase
    {
        ...
        /// <returns></returns>
        protected static bool PointsEqual(IntPoint2D pt1, IntPoint2D pt2)
        {
            return (pt1.X == pt2.X && pt1.Y == pt2.Y);
        }
        //------------------------------------------------------------------------------

        internal bool PointIsVertex(IntPoint2D pt, OutPt pp)
        {
            OutPt pp2 = pp;
            do
            {
        ...
        internal bool PointInPolygon(IntPoint2D pt, OutPt pp, bool UseFulllongRange)
        {
            OutPt pp2 = pp;
            bool result = false;
            if (UseFulllongRange)
        ...
        /// <returns></returns>
        protected bool SlopesEqual(IntPoint2D pt1, IntPoint2D pt2,
            IntPoint2D pt3, bool UseFullRange)
        {
            if (UseFullRange)
        ...
        /// <returns></returns>
        protected bool SlopesEqual(IntPoint2D pt1, IntPoint2D pt2,
            IntPoint2D pt3, IntPoint2D pt4, bool UseFullRange)
        {
            if (UseFullRange)
        ...
        public bool AddPolygon(Polygon pg, PolyType polyType)
        {
            ...
            p.Add(new IntPoint2D(pg[0].X, pg[0].Y));
            int j = 0;
            for (int i = 1; i < len; ++i)
            {
                ...
                if (j < p.Count)
                    ...
                    p.Add(new IntPoint2D(pg[i].X, pg[i].Y));
            }
            if (j < 2) return false;
        ...
        private void InitEdge(TEdge e, TEdge eNext,
          TEdge ePrev, IntPoint2D pt, PolyType polyType)
        {
            e.next = eNext;
            e.prev = ePrev;
            e.xcurr = pt.X;
        ...
        {
            IntRegion2D result = new IntRegion2D();
            LocalMinima lm = m_MinimaList;
