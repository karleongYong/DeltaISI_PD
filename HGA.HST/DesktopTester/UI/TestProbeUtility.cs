...
namespace DesktopTester.UI
{
    public partial class frmMain
    {
        ...
        private bool IsServoCalibrationTempered = false;
        private bool IsConnectedToDesktopTester = false;
        private XOrZ2Command PrecisorXOrTestProbeZCommand = XOrZ2Command.Unknown;
        private UpOrDownTab TabType = UpOrDownTab.Unknown;
        ...
        private TestProbe80SetDesktopTesterConnect TestProbe80SetDesktopTesterConnect;
        private TestProbe81SetDesktopTesterHomingAxis TestProbe81SetDesktopTesterHomingAxis;
        private TestProbe83SetDesktopTesterServoAbsMoveAxis TestProbe83SetDesktopTesterServoAbsMoveAxis;
        private TestProbe84SetDesktopTesterServoRelMoveAxis TestProbe84SetDesktopTesterServoRelMoveAxis;
        private TestProbe86SetDesktopTesterServoTeachPosition TestProbe86SetDesktopTesterServoTeachPosition;
        private TestProbe87SetDesktopTesterMoveToTaughtServoPosition TestProbe87SetDesktopTesterMoveToTaughtServoPosition;
        private TestProbe89SetDesktopTesterOutput TestProbe89SetDesktopTesterOutput;
        private TestProbe91SetDesktopLoadHGA TestProbe91SetDesktopLoadHGA;
        private TestProbe94SetDesktopJog TestProbe94SetDesktopJog;
        private TestProbe95SetDesktopJogStop TestProbe95SetDesktopJogStop;
        private TestProbe96SetDesktopPrecisorVacuumOn TestProbe96SetDesktopPrecisorVacuumOn;
        ...
        /*********************************************************************************************************************************************/
        // Desktop Tester commands
        private TestProbe80GetDesktopTesterConnect TestProbe80GetDesktopTesterConnect;
        private TestProbe85GetDesktopTesterServoPositions TestProbe85GetDesktopTesterServoPositions;
        private TestProbe86GetDesktopTesterServoTeachPosition TestProbe86GetDesktopTesterServoTeachPosition;
        private TestProbe90GetDesktopTesterAllInputs TestProbe90GetDesktopTesterAllInputs;
        ...
        private bool constructAndSendWriteDataBuffer()
        {
            ...
            {
                DataReceived = true;
                testProbeComPort.DiscardOutBuffer();
            ...
            try
            {
                ...
                /* Special handling of jog and jog stop commands because the firmware does not send back their acknowledgment. So we need to
                 * remove them from the OutgoingTestProbeDataAPIs queue. */
                if (APICommand.CommandID == TestProbeAPICommand.HST_desktop_tester_jog_Message_ID ||
                    APICommand.CommandID == TestProbeAPICommand.HST_desktop_tester_jog_stop_Message_ID)
                {
        ...
        private void testProbeComPort_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                uint CalculatedMessageCheckSum = 0;
                ...
                /*CalculatedMessageCheckSum += readDataBuffer[2] + readDataBuffer[3] + readDataBuffer[4] + readDataBuffer[5];

                for (int i = 6; i <= CommandSize; i++)
                {
                    CalculatedMessageCheckSum += readDataBuffer[i];
                }

                if (CalculatedMessageCheckSum != readDataBuffer[CommandSize + 1])
                {
                    Notify.PopUpError("Checksum Mismatch", String.Format("The calculated checksum of {0} does not tally with the read in value of {1}.", CalculatedMessageCheckSum, readDataBuffer[CommandSize + 1]));
                    Log.Info("Checksum Mismatch", String.Format("The calculated checksum of {0} does not tally with the read in value of {1}.", CalculatedMessageCheckSum, readDataBuffer[CommandSize + 1]));
                ...
                switch (CommandID)
                {
                    ...
                    case (byte)MessageID.HST_desktop_tester_set_homing_axis: // 81
                    case (byte)MessageID.HST_desktop_tester_set_servo_off: // 82
                    case (byte)MessageID.HST_desktop_tester_set_servo_abs_move_axis: // 83
                    case (byte)MessageID.HST_desktop_tester_set_servo_rel_move_axis: // 84
                    case (byte)MessageID.HST_desktop_tester_move_to_taught_servo_position: // 87
                    case (byte)MessageID.HST_desktop_tester_save_taught_position: // 88
                    case (byte)MessageID.HST_desktop_tester_set_output: // 89
                    case (byte)MessageID.HST_desktop_tester_load_hga: // 91
                    case (byte)MessageID.HST_desktop_tester_eject_hga: // 92
                    case (byte)MessageID.HST_desktop_tester_jog: // 94
                    case (byte)MessageID.HST_desktop_tester_jog_stop: // 95
                    case (byte)MessageID.HST_desktop_tester_precisor_vac_on: // 96
                    ...
                    case (byte)MessageID.HST_get_short_detection: // 10
                        ...
                        UpdateGUIDisplay(GUIPage.DesktopTestsTabPage, CommandID, readDataBuffer);
                        break;
                    ...
                    case (byte)MessageID.HST_get_precisor_cap_compensation: // 53
                        ...
                        break;
                    case (byte)MessageID.HST_desktop_tester_connect: // 80
                        TestProbe80GetDesktopTesterConnect = TestProbe80GetDesktopTesterConnect.ReadStruct(readDataBuffer);
                        CommandParameterSize = System.Runtime.InteropServices.Marshal.SizeOf(typeof(TestProbe80GetDesktopTesterConnect));
                        UpdateGUIDisplay(GUIPage.ServoCalibrationTabPage, CommandID, readDataBuffer);
                        UpdateGUIDisplay(GUIPage.DesktopTestsTabPage, CommandID, readDataBuffer);
                        break;
                    case (byte)MessageID.HST_desktop_tester_get_servo_positions: // 85
                        TestProbe85GetDesktopTesterServoPositions = TestProbe85GetDesktopTesterServoPositions.ReadStruct(readDataBuffer);
                        CommandParameterSize = System.Runtime.InteropServices.Marshal.SizeOf(typeof(TestProbe85GetDesktopTesterServoPositions));
                        UpdateGUIDisplay(GUIPage.ServoCalibrationTabPage, CommandID, readDataBuffer);
                        break;
                    case (byte)MessageID.HST_desktop_tester_teach_servo_position: // 86
                        TestProbe86GetDesktopTesterServoTeachPosition = TestProbe86GetDesktopTesterServoTeachPosition.ReadStruct(readDataBuffer);
                        CommandParameterSize = System.Runtime.InteropServices.Marshal.SizeOf(typeof(TestProbe86GetDesktopTesterServoTeachPosition));
                        UpdateGUIDisplay(GUIPage.ServoCalibrationTabPage, CommandID, readDataBuffer);
                        break;
                    case (byte)MessageID.HST_desktop_tester_get_all_inputs: // 90
                        TestProbe90GetDesktopTesterAllInputs = TestProbe90GetDesktopTesterAllInputs.ReadStruct(readDataBuffer);
                        CommandParameterSize = System.Runtime.InteropServices.Marshal.SizeOf(typeof(TestProbe90GetDesktopTesterAllInputs));
                        UpdateGUIDisplay(GUIPage.ServoCalibrationTabPage, CommandID, readDataBuffer);
                        break;
                    case (byte)MessageID.HST_desktop_tester_disconnect: // 93
                        TestProbeGetStatusAndErrorCode = TestProbeGetStatusAndErrorCode.ReadStruct(readDataBuffer);
                        CommandParameterSize = System.Runtime.InteropServices.Marshal.SizeOf(typeof(TestProbeGetStatusAndErrorCode));
                        IsConnectedToDesktopTester = false;
                        tabPageServoCalibration.Refresh();
                        tabPageDesktopTests.Refresh();
                        break;
                }
                ...
                CalculatedMessageCheckSum = (uint)CommandType + (uint)CommandID + MessageCheckSum;
            ...
            {
                Notify.PopUpError("Serial Port Transmission Error", "Error in testProbeComPort_DataReceived() function: " + ex.Message + ".");
                Log.Error("Serial Port Transmission Error", "Error in testProbeComPort_DataReceived() function: " + ex.Message + ".");
                testProbeComPort.DiscardOutBuffer();
                enableAllAPIButtons();
                return;
            }
        ...
        {           
            if (this.InvokeRequired)
            {
                ...
                this.BeginInvoke(d, new object[] { page, CommandID, data });
            }
            else
            {
                switch (page)
                {
                    ...
                    case GUIPage.ServoCalibrationTabPage:
                        if (CommandID == (byte)MessageID.HST_desktop_tester_connect)
                        {                            
                            byte[] byteArray = BitConverter.GetBytes(TestProbe80GetDesktopTesterConnect.Password());
                            if (byteArray[0] == 0x5A && byteArray[1] == 0x5A)
                            {
                                IsConnectedToDesktopTester = true;
                            }
                            else
                            {
                                IsConnectedToDesktopTester = false;
                        ...
                        if (CommandID == (byte)MessageID.HST_desktop_tester_get_servo_positions)
                        {
                            txtServoCalibrationPrecisorActuatorsXCurrentPosition.Text = TestProbe85GetDesktopTesterServoPositions.XPosition().ToString();
                            txtServoCalibrationPrecisorActuatorsYCurrentPosition.Text = TestProbe85GetDesktopTesterServoPositions.YPosition().ToString();
                            txtServoCalibrationPrecisorActuatorsThetaCurrentPosition.Text = TestProbe85GetDesktopTesterServoPositions.ThetaPosition().ToString();
                            txtServoCalibrationTestProbeZCurrentPosition.Text = TestProbe85GetDesktopTesterServoPositions.Z2Position().ToString();
                        }
                        if (CommandID == (byte)MessageID.HST_desktop_tester_teach_servo_position)
                        {
                            // Precisor Nest
                            if (PrecisorXOrTestProbeZCommand == XOrZ2Command.X)
                            {
                                txtServoCalibrationPrecisorActuatorsXCurrentPosition.Text = TestProbe86GetDesktopTesterServoTeachPosition.XOrZ2Position().ToString();
                                txtServoCalibrationPrecisorActuatorsYCurrentPosition.Text = TestProbe86GetDesktopTesterServoTeachPosition.YPosition().ToString();
                                txtServoCalibrationPrecisorActuatorsThetaCurrentPosition.Text = TestProbe86GetDesktopTesterServoTeachPosition.ThetaPosition().ToString();  
                            }
                                // Test Probe
                            else if (PrecisorXOrTestProbeZCommand == XOrZ2Command.Z2)
                            {
                                txtServoCalibrationTestProbeZCurrentPosition.Text = TestProbe86GetDesktopTesterServoTeachPosition.XOrZ2Position().ToString();
                            }                                                      
                        }
                        if (CommandID == (byte)MessageID.HST_desktop_tester_get_all_inputs)
                        {
                            txtServoCalibrationInput1.Text = TestProbe90GetDesktopTesterAllInputs.EStopInputStatus.ToString();
                            txtServoCalibrationInput2.Text = TestProbe90GetDesktopTesterAllInputs.FlattenerRetract1SensorInput.ToString();
                            txtServoCalibrationInput3.Text = TestProbe90GetDesktopTesterAllInputs.FlattenerExtend1SensorInput.ToString();
                            txtServoCalibrationInput4.Text = TestProbe90GetDesktopTesterAllInputs.Reserved1.ToString();
                            txtServoCalibrationInput5.Text = TestProbe90GetDesktopTesterAllInputs.Reserved2.ToString();
                            txtServoCalibrationInput6.Text = TestProbe90GetDesktopTesterAllInputs.Reserved3.ToString();
                            txtServoCalibrationInput7.Text = TestProbe90GetDesktopTesterAllInputs.Reserved4.ToString();
                            txtServoCalibrationInput8.Text = TestProbe90GetDesktopTesterAllInputs.Reserved5.ToString();
                        }

                        break;

                    case GUIPage.DesktopTestsTabPage:
                        if (CommandID == (byte)MessageID.HST_desktop_tester_connect)
                        {
                            byte[] byteArray = BitConverter.GetBytes(TestProbe80GetDesktopTesterConnect.Password());
                            if (byteArray[0] == 0x5A && byteArray[1] == 0x5A)
                        ...
                        ShortDetection DesktopTestsHGA1Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA2Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA3Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA4Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA5Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA6Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA7Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA8Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA9Short = ShortDetection.NoTest;
                        ShortDetection DesktopTestsHGA10Short = ShortDetection.NoTest;
                        ...
                        if (CommandID == (byte)MessageID.HST_get_res_results)
                        {
                            ...
                            double HGA1R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA1R1Resistance() / 1000.0);
                            double HGA1R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA1R2Resistance() / 1000.0);
                            txtDesktopTestsHGA1Ch1Res.Text = HGA1WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA1Ch2Res.Text = HGA1TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA1Ch3Res.Text = HGA1WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA1Ch4Res.Text = HGA1RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA1Ch5Res.Text = HGA1R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA1Ch6Res.Text = HGA1R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA2R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA2R1Resistance() / 1000.0);
                            double HGA2R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA2R2Resistance() / 1000.0);
                            txtDesktopTestsHGA2Ch1Res.Text = HGA2WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA2Ch2Res.Text = HGA2TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA2Ch3Res.Text = HGA2WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA2Ch4Res.Text = HGA2RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA2Ch5Res.Text = HGA2R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA2Ch6Res.Text = HGA2R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA3R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA3R1Resistance() / 1000.0);
                            double HGA3R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA3R2Resistance() / 1000.0);
                            txtDesktopTestsHGA3Ch1Res.Text = HGA3WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA3Ch2Res.Text = HGA3TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA3Ch3Res.Text = HGA3WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA3Ch4Res.Text = HGA3RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA3Ch5Res.Text = HGA3R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA3Ch6Res.Text = HGA3R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA4R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA4R1Resistance() / 1000.0);
                            double HGA4R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA4R2Resistance() / 1000.0);
                            txtDesktopTestsHGA4Ch1Res.Text = HGA4WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA4Ch2Res.Text = HGA4TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA4Ch3Res.Text = HGA4WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA4Ch4Res.Text = HGA4RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA4Ch5Res.Text = HGA4R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA4Ch6Res.Text = HGA4R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA5R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA5R1Resistance() / 1000.0);
                            double HGA5R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA5R2Resistance() / 1000.0);
                            txtDesktopTestsHGA5Ch1Res.Text = HGA5WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA5Ch2Res.Text = HGA5TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA5Ch3Res.Text = HGA5WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA5Ch4Res.Text = HGA5RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA5Ch5Res.Text = HGA5R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA5Ch6Res.Text = HGA5R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA6R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA6R1Resistance() / 1000.0);
                            double HGA6R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA6R2Resistance() / 1000.0);
                            txtDesktopTestsHGA6Ch1Res.Text = HGA6WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA6Ch2Res.Text = HGA6TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA6Ch3Res.Text = HGA6WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA6Ch4Res.Text = HGA6RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA6Ch5Res.Text = HGA6R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA6Ch6Res.Text = HGA6R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA7R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA7R1Resistance() / 1000.0);
                            double HGA7R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA7R2Resistance() / 1000.0);
                            txtDesktopTestsHGA7Ch1Res.Text = HGA7WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA7Ch2Res.Text = HGA7TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA7Ch3Res.Text = HGA7WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA7Ch4Res.Text = HGA7RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA7Ch5Res.Text = HGA7R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA7Ch6Res.Text = HGA7R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA8R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA8R1Resistance() / 1000.0);
                            double HGA8R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA8R2Resistance() / 1000.0);
                            txtDesktopTestsHGA8Ch1Res.Text = HGA8WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA8Ch2Res.Text = HGA8TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA8Ch3Res.Text = HGA8WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA8Ch4Res.Text = HGA8RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA8Ch5Res.Text = HGA8R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA8Ch6Res.Text = HGA8R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA9R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA9R1Resistance() / 1000.0);
                            double HGA9R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA9R2Resistance() / 1000.0);
                            txtDesktopTestsHGA9Ch1Res.Text = HGA9WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA9Ch2Res.Text = HGA9TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA9Ch3Res.Text = HGA9WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA9Ch4Res.Text = HGA9RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA9Ch5Res.Text = HGA9R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA9Ch6Res.Text = HGA9R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            ...
                            double HGA10R1Resistance = (TestProbe11GetAllHGAResistanceResults.HGA10R1Resistance() / 1000.0);
                            double HGA10R2Resistance = (TestProbe11GetAllHGAResistanceResults.HGA10R2Resistance() / 1000.0);
                            txtDesktopTestsHGA10Ch1Res.Text = HGA10WriterResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA10Ch2Res.Text = HGA10TAResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA10Ch3Res.Text = HGA10WHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA10Ch4Res.Text = HGA10RHResistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA10Ch5Res.Text = HGA10R1Resistance.ToString("F3", CultureInfo.InvariantCulture);
                            txtDesktopTestsHGA10Ch6Res.Text = HGA10R2Resistance.ToString("F3", CultureInfo.InvariantCulture);
                        }

                        if (CommandID == (byte)MessageID.HST_get_cap_results)
                        {
                            txtDesktopTestsHGA1Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA1Capacitance1().ToString();
                            txtDesktopTestsHGA1Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA1Capacitance2().ToString();

                            txtDesktopTestsHGA2Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA2Capacitance1().ToString();
                            txtDesktopTestsHGA2Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA2Capacitance2().ToString();

                            txtDesktopTestsHGA3Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA3Capacitance1().ToString();
                            txtDesktopTestsHGA3Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA3Capacitance2().ToString();

                            txtDesktopTestsHGA4Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA4Capacitance1().ToString();
                            txtDesktopTestsHGA4Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA4Capacitance2().ToString();

                            txtDesktopTestsHGA5Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA5Capacitance1().ToString();
                            txtDesktopTestsHGA5Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA5Capacitance2().ToString();

                            txtDesktopTestsHGA6Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA6Capacitance1().ToString();
                            txtDesktopTestsHGA6Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA6Capacitance2().ToString();

                            txtDesktopTestsHGA7Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA7Capacitance1().ToString();
                            txtDesktopTestsHGA7Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA7Capacitance2().ToString();

                            txtDesktopTestsHGA8Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA8Capacitance1().ToString();
                            txtDesktopTestsHGA8Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA8Capacitance2().ToString();

                            txtDesktopTestsHGA9Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA9Capacitance1().ToString();
                            txtDesktopTestsHGA9Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA9Capacitance2().ToString();

                            txtDesktopTestsHGA10Ch1Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA10Capacitance1().ToString();
                            txtDesktopTestsHGA10Ch2Capa.Text = TestProbe12GetAllHGACapacitanceResults.HGA10Capacitance2().ToString();
                        }

                        if (CommandID == (byte)MessageID.HST_get_cap_secondary_results)
                        {
                            txtDesktopTestsHGA1Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA1ESR1().ToString();
                            txtDesktopTestsHGA1Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA1ESR2().ToString();

                            txtDesktopTestsHGA2Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA2ESR1().ToString();
                            txtDesktopTestsHGA2Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA2ESR2().ToString();

                            txtDesktopTestsHGA3Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA3ESR1().ToString();
                            txtDesktopTestsHGA3Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA3ESR2().ToString();

                            txtDesktopTestsHGA4Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA4ESR1().ToString();
                            txtDesktopTestsHGA4Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA4ESR2().ToString();

                            txtDesktopTestsHGA5Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA5ESR1().ToString();
                            txtDesktopTestsHGA5Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA5ESR2().ToString();

                            txtDesktopTestsHGA6Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA6ESR1().ToString();
                            txtDesktopTestsHGA6Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA6ESR2().ToString();

                            txtDesktopTestsHGA7Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA7ESR1().ToString();
                            txtDesktopTestsHGA7Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA7ESR2().ToString();

                            txtDesktopTestsHGA8Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA8ESR1().ToString();
                            txtDesktopTestsHGA8Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA8ESR2().ToString();

                            txtDesktopTestsHGA9Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA9ESR1().ToString();
                            txtDesktopTestsHGA9Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA9ESR2().ToString();

                            txtDesktopTestsHGA10Ch1ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA10ESR1().ToString();
                            txtDesktopTestsHGA10Ch2ESR.Text = TestProbe34GetAllHGACapacitanceSecondaryResults.HGA10ESR2().ToString();
                        }                        
                        ...
                        if (CommandID == (byte)MessageID.HST_get_short_detection)
                        {
                            if (TestProbe10GetAllHGAShortDetection.HGA1WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA1R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA1Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA1Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA2WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA2R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA2Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA2Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA3WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA3R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA3Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA3Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA4WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA4R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA4Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA4Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA5WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA5R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA5Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA5Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA6WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA6R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA6Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA6Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA7WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA7R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA7Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA7Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA8WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA8R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA8Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA8Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA9WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA9R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA9Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA9Short = ShortDetection.Open;
                            }
                            ...
                            if (TestProbe10GetAllHGAShortDetection.HGA10WPlusPad == (int)ShortDetection.Short ||
                                ...
                                TestProbe10GetAllHGAShortDetection.HGA10R2MinusPad == (int)ShortDetection.Short)
                            {
                                DesktopTestsHGA10Short = ShortDetection.Short;
                            }
                            else
                            {
                                DesktopTestsHGA10Short = ShortDetection.Open;
                            }

                            // HGA1
                            if (DesktopTestsHGA1Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA1Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA1Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA1Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA1Short.Text = "No Test";
                            }

                            // HGA2
                            if (DesktopTestsHGA2Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA2Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA2Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA2Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA2Short.Text = "No Test";
                            }

                            // HGA3
                            if (DesktopTestsHGA3Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA3Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA3Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA3Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA3Short.Text = "No Test";
                            }

                            // HGA4
                            if (DesktopTestsHGA4Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA4Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA4Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA4Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA4Short.Text = "No Test";
                            }

                            // HGA5
                            if (DesktopTestsHGA5Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA5Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA5Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA5Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA5Short.Text = "No Test";
                            }

                            // HGA6
                            if (DesktopTestsHGA6Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA6Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA6Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA6Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA6Short.Text = "No Test";
                            }

                            // HGA7
                            if (DesktopTestsHGA7Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA7Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA7Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA7Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA7Short.Text = "No Test";
                            }

                            // HGA8
                            if (DesktopTestsHGA8Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA8Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA8Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA8Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA8Short.Text = "No Test";
                            }

                            // HGA9
                            if (DesktopTestsHGA9Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA9Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA9Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA9Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA9Short.Text = "No Test";
                            }

                            // HGA10
                            if (DesktopTestsHGA10Short == ShortDetection.Short)
                            {
                                txtDesktopTestsHGA10Short.Text = "Short";
                            }
                            else if (DesktopTestsHGA10Short == ShortDetection.Open)
                            {
                                txtDesktopTestsHGA10Short.Text = "Open";
                            }
                            else
                            {
                                txtDesktopTestsHGA10Short.Text = "No Test";
                            }
                        }
                        break;
                }
