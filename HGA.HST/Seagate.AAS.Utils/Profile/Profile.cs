...
{		
	...
	public abstract class Profile : IProfile
	{
		...
		public abstract object GetValue(string section, string entry, out bool notFound);
		...
		/// <seealso cref="SetValue" />
		/// <seealso cref="HasEntry" />
        public virtual string GetValue(string section, string entry, string defaultValue, out bool notFound)
        {
            object value = GetValue(section, entry, out notFound);
            if(value == null)
            {
                notFound = true;
                return defaultValue;
		...
		/// <seealso cref="SetValue" />
		/// <seealso cref="HasEntry" />
		public virtual int GetValue(string section, string entry, int defaultValue, out bool notFound)
		{
			object value = GetValue(section, entry, out notFound);
            if (value == null)
            {
		...
		/// <seealso cref="SetValue" />
		/// <seealso cref="HasEntry" />
		public virtual double GetValue(string section, string entry, double defaultValue, out bool notFound)
		{
			object value = GetValue(section, entry, out notFound);
            if (value == null)
            {
		...
		/// <seealso cref="SetValue" />
		/// <seealso cref="HasEntry" />
		public virtual bool GetValue(string section, string entry, bool defaultValue, out bool notFound)
		{
			object value = GetValue(section, entry, out notFound);
            if (value == null)
            {
		...
		public virtual void Test(bool cleanup)
		{
			...
			try
			{
				...
				task = "retrieving a null section";
					...
					try
					{
						GetValue(null, "Test", out notFound); 
				...
				task = "creating a ReadOnly clone of the object";
					...
					dValue = roProfile.GetValue(section, "Double entry", 0.0, out notFound);
					if (dValue != 17.95)
