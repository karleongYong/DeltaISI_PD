...
using BenchTestsTool.UserControls;
        ...
        private void UserChanged(object sender, EventArgs e)
        {
            try
            {
                UIUtility.Invoke(this, () =>
                {
                    ...
                    TabPage tabPageApplicationSettings = MainForm.getTabControl().TabPages[5] as TabPage;         
                    ...
                    switch (level)
                    {
                        ...
                        case UserLevel.Operator:
                            userIcon.Image = ManufacturingTestTool.Properties.Resources.UserOperator;                            
                            (MainForm.getTabControl().TabPages[3] as Control).Enabled = false;  // Calibration tab
                            (MainForm.getTabControl().TabPages[4] as Control).Enabled = false;  // Debug tab
                            (MainForm.getTabControl().TabPages[5] as Control).Enabled = false;   // Application Settings tab
                        ...
                        case UserLevel.Engineer:
                            userIcon.Image = ManufacturingTestTool.Properties.Resources.UserEngineer;                            
                            (MainForm.getTabControl().TabPages[3] as Control).Enabled = false;  // Calibration tab
                            (MainForm.getTabControl().TabPages[4] as Control).Enabled = true;   // Debug tab
                            (MainForm.getTabControl().TabPages[5] as Control).Enabled = true;   // Application Settings tab                                               
                            (tabControlApplicationSettings.TabPages[0] as Control).Enabled = true;  // COM Port tab
                            (tabControlApplicationSettings.TabPages[1] as Control).Enabled = false; // User Accounts tab                            
                            break;
                        case UserLevel.Administrator:
                            userIcon.Image = ManufacturingTestTool.Properties.Resources.UserAdministrator;                            
                            (MainForm.getTabControl().TabPages[3] as Control).Enabled = true;   // Calibration tab
                            (MainForm.getTabControl().TabPages[4] as Control).Enabled = true;   // Debug tab
                            (MainForm.getTabControl().TabPages[5] as Control).Enabled = true;   // Application Settings tab                            
                            (tabControlApplicationSettings.TabPages[0] as Control).Enabled = true;  // COM Port tab
